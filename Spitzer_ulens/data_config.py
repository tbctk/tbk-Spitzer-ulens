...
def read_centroid_data(dirpath,event_coords,origin=1,timerange=None):
    """
    Reads each corrected BCD (CBCD) file in the provided data root directory.
    The directory structure must be as downloaded from ipac website: dirpath/xxxxx/ch1/bcd/file_name_cbcd.fits
    Args:
    ...
    for filename in os.listdir(dirpath):
        if filename.startswith('SPITZER_I1_') and filename.endswith('_cbcd.fits') and not filename.endswith('0_0000_1_cbcd.fits'):
            label,time,xp,yp,fname,expid = get_centroid_data_from_file(os.path.join(dirpath,filename),event_coords,origin=origin)
            if (timerange is None) or (time > timerange[0] and time < timerange[1]):
                data.append(np.asarray([label,time,xp,yp,fname,expid]))
    return np.asarray(data)

def generate_centroid_file(dirpath,event_coords,destpath='',origin=1,**kwargs):
    """
    Reads each corrected BCD (CBCD) file in the provided data root directory and writes the output data to output file destpath/centroid.out
    The directory structure must be as downloaded from ipac website: dirpath/xxxxx/ch1/bcd/file_name_cbcd.fits
    Args:
        dirpath (string): path to data root directory
        event_coords (tuple of strings): (ra,dec) where ra='hh:mm:ss.ss' and dec='dd:mm:ss.ss'
...
def target_central_px(XDATA, YDATA):
    ...
    Returns:
        array: x-coordinate of central pixel
        array: y-coordinate or central pixel
    """
    # number of dithers
...
def target_image_square(evt, xcent, ycent, CBCD, CBUNC, box=5):
    ...
    Returns:
        list: list of 3D-array (image flux stacks) per dither positions
        list: list of 3D-array (image uncertainty stacks) per dither positions
    """
